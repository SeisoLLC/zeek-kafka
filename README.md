...
1. Install the plugin using `zkg install`.
    ...
    Installed "zeek/seisollc/zeek-kafka" (1.2.0-rc1)
...
1. Run the following command to ensure that the plugin was installed successfully.
    ...
    ```
    $ zeek -N Seiso::Kafka
    Seiso::Kafka - Writes logs to Kafka (dynamic, version 1.2.0-rc1)
    ```
